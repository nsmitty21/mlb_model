...
from config import BET_LOG_CSV, LIVE_DIR, LOGS_DIR, MODEL_DIR, RETRAIN_HOUR

PICKS_PATH = os.path.join(LIVE_DIR, "picks_today.json")
LINES_PATH = os.path.join(LIVE_DIR, "today_lines.csv")
...
def load_bet_log():
    if not Path(BET_LOG_CSV).exists(): return []
    try:
...
def compute_stats(bet_log):
    ...
    if graded:
        ...
        for b in reversed(graded):
            if b["result"]==last: streak+=1
            else: break
        if last=="L": streak=-streak
    by_market = {}
    for b in graded:
        m = b.get("market","unknown")
        if m not in by_market: by_market[m]={"wins":0,"losses":0,"pnl":0}
        by_market[m]["wins" if b["result"]=="W" else "losses"]+=1
        by_market[m]["pnl"]+=float(b.get("pnl_units",0) or 0)
    return {
        "total_bets":len(bet_log),"wins":wins,"losses":losses,"pending":pending,
        "win_pct":round(wins/len(graded)*100,1) if graded else 0,
        "total_units":round(total_pnl,2),"roi":round(roi,1),
        "units_by_market":by_market,"streak":streak,
    }
...
def trigger_page_update():
    """Update the public picks page after picks or results change."""
    try:
        import generate_mlb_page
        generate_mlb_page.main()
    except Exception as e:
        print(f"  [warn] Page update skipped: {e}")
...
def log_bet():
    ...
    # Update public page
    threading.Thread(target=trigger_page_update, daemon=True).start()
